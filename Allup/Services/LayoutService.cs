...
namespace Allup.Services
{
    public class LayoutService : ILayoutService
    {
        ...
        {
            return await _appDbContext.Categories.Include(c => c.Children).Where(c => c.IsMain == true  && c.IsDeleted == false).ToListAsync();
        }

        public async Task<IDictionary<string , string>> GetSettings()
        {
            IDictionary<string, string> settings = await _appDbContext.Setting.ToDictionaryAsync(s => s.Key, s => s.Value);
